...
def main_entry_point() -> None:
    ...
    log.debug("Running Next EmbyCon: {0}", ClientInformation().get_version())
    ...
    log.debug("Next EmbyCon -> Mode: {0}", mode)
    log.debug("Next EmbyCon -> URL: {0}", param_url)
    ...
    log.debug("===== Next EmbyCon FINISHED =====")
...
def get_params() -> dict[str, str]:
    ...
    log.debug("Next EmbyCon -> Detected parameters: {0}", param)
    return param
...
def show_menu(params: dict[str, str]) -> None:
    ...
    if selected_action == "play":
        ...
        action_url = (
            "plugin://plugin.video.nextembycon/?url="
...
def play_action(params: dict[str, str]) -> None:
    ...
    log.info("Sending nextembycon_play_action : {0}", play_info)
...
def play_item_trailer(item_id: str) -> None:
    ...
    if resp > -1:
        ...
        if trailer.get("type") == "local":
            ...
            # send_event_notification("nextembycon_play_youtube_trailer_action", play_info)
