...
def do_user_change(menu_params: dict[str, str]) -> None:
    ...
    if current_username != user_name:
        ...
        if "estuary_nextembycon" in xbmc.getSkinDir():
...
def display_main_menu() -> None:
    ...
    add_menu_directory_item(
        string_load(30460),
        "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=global_list_movies",
    )
    add_menu_directory_item(
        string_load(30461),
        "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=global_list_tvshows",
    )
    add_menu_directory_item(
        string_load(30465),
        "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=favorites",
    )
    add_menu_directory_item(
        string_load(30406),
        "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=library",
    )
    add_menu_directory_item(
        string_load(30463),
        "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=show_custom_widgets",
    )
    add_menu_directory_item(
        string_load(30462),
        "plugin://plugin.video.nextembycon/?mode=SEARCH",
    )
    add_menu_directory_item(
        string_load(30464),
        "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=addon_items",
    )
    add_menu_directory_item(
        "Custom Nodes",
        "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=custom_nodes",
    )
...
def show_custom_nodes(_params: dict[str, str]) -> None:
    ...
    add_menu_directory_item(
        "[Edit Nodes]",
        "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=create_new_node",
        folder=False,
    )
    ...
    for node_name in node_names:
        ...
        add_menu_directory_item(
            node_name,
            "plugin://plugin.video.nextembycon/?mode=SHOW_NODE_CONTENT&node_name="
            + encoded_name,
        )
...
def display_addon_menu(_params: dict[str, str]) -> None:
    add_menu_directory_item(
        string_load(30246), "plugin://plugin.video.nextembycon/?mode=SEARCH"
    )
    add_menu_directory_item(
        string_load(30017), "plugin://plugin.video.nextembycon/?mode=SHOW_SERVER_SESSIONS"
    )
    add_menu_directory_item(
        string_load(30012), "plugin://plugin.video.nextembycon/?mode=CHANGE_USER"
    )
    add_menu_directory_item(
        "Show Users", "plugin://plugin.video.nextembycon/?mode=SHOW_USERS"
    )
    add_menu_directory_item(
        string_load(30011), "plugin://plugin.video.nextembycon/?mode=DETECT_SERVER_USER"
    )
    add_menu_directory_item(
        string_load(30435),
        "plugin://plugin.video.nextembycon/?mode=DETECT_CONNECTION_SPEED",
    )
    add_menu_directory_item(
        string_load(30254), "plugin://plugin.video.nextembycon/?mode=SHOW_SETTINGS"
    )
    add_menu_directory_item(
        string_load(30395), "plugin://plugin.video.nextembycon/?mode=CLEAR_CACHE"
    )
    add_menu_directory_item(
        string_load(30293), "plugin://plugin.video.nextembycon/?mode=CACHE_ARTWORK"
    )
    add_menu_directory_item(
        "List Performance Profiles",
        "plugin://plugin.video.nextembycon/?mode=LIST_AVAILABLE_PROFILES",
    )
    add_menu_directory_item(
        "Clone default skin", "plugin://plugin.video.nextembycon/?mode=CLONE_SKIN"
    )
...
def display_tvshow_type(_menu_params: dict[str, str], view: dict[str, str]) -> None:
    ...
    # TV Show Genres
    path = "plugin://plugin.video.nextembycon/?mode=GENRES&item_type=tvshow"
    if view is not None:
        path += "&parent_id=" + view.get("Id", "none")
    add_menu_directory_item(view_name + string_load(30325), path)

    # TV Show Alpha picker
    path = "plugin://plugin.video.nextembycon/?mode=TVSHOW_ALPHA"
    if view is not None:
        path += "&parent_id=" + view.get("Id", "none")
    add_menu_directory_item(view_name + string_load(30404), path)

    # Tv Show Pages
    path = "plugin://plugin.video.nextembycon/?mode=TVSHOW_PAGES"
    if view is not None:
        path += "&parent_id=" + view.get("Id", "none")
...
def display_movies_type(_menu_params: dict[str, str], view: dict[str, str]) -> None:
    ...
    # Genres
    path = "plugin://plugin.video.nextembycon/?mode=GENRES&item_type=movie"
    if view is not None:
        path += "&parent_id=" + view.get("Id", "none")
    add_menu_directory_item(view_name + string_load(30325), path)

    # Pages
    path = "plugin://plugin.video.nextembycon/?mode=MOVIE_PAGES"
    if view is not None:
        path += "&parent_id=" + view.get("Id", "none")
    add_menu_directory_item(view_name + string_load(30397), path)

    # Alpha Picker
    path = "plugin://plugin.video.nextembycon/?mode=MOVIE_ALPHA"
    if view is not None:
        path += "&parent_id=" + view.get("Id", "none")
    add_menu_directory_item(view_name + string_load(30404), path)

    # Years
    path = "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=show_movie_years"
    if view is not None:
        path += "&parent_id=" + view.get("Id", "none")
    add_menu_directory_item(view_name + string_load(30411), path)

    # Decades
    path = "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=show_movie_years&group=true"
    if view is not None:
        path += "&parent_id=" + view.get("Id", "none")
    add_menu_directory_item(view_name + string_load(30412), path)

    # Tags
    path = "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=show_movie_tags"
    if view is not None:
        path += "&parent_id=" + view.get("Id", "none")
...
def display_library_views(_params: dict[str, str]) -> None:
    ...
    for view in views:
        ...
        if collection_type in view_types or item_type == "Channel":
            ...
            plugin_path = (
                "plugin://plugin.video.nextembycon/?mode=SHOW_ADDON_MENU&type=library_item&view_id="
                + view.get("Id")
            )
...
def show_widgets() -> None:
    ...
    add_menu_directory_item(
        "All Movies",
        "plugin://plugin.video.nextembycon/?mode=SHOW_CONTENT&item_type=movie&media_type=movies",
    )

    add_menu_directory_item(
        "All Shows",
        "plugin://plugin.video.nextembycon/?mode=SHOW_CONTENT&item_type=series&media_type=tvshows",
    )

    add_menu_directory_item(
        string_load(30257) + " (" + show_x_filtered_items + ")",
        "plugin://plugin.video.nextembycon/?mode=WIDGET_CONTENT&type=recent_movies",
    )
    add_menu_directory_item(
        string_load(30258) + " (" + show_x_filtered_items + ")",
        "plugin://plugin.video.nextembycon/?mode=WIDGET_CONTENT&type=inprogress_movies",
    )
    add_menu_directory_item(
        string_load(30269) + " (" + show_x_filtered_items + ")",
        "plugin://plugin.video.nextembycon/?mode=WIDGET_CONTENT&type=random_movies",
    )
    add_menu_directory_item(
        string_load(30403) + " (" + show_x_filtered_items + ")",
        "plugin://plugin.video.nextembycon/?mode=WIDGET_CONTENT&type=movie_recommendations",
    )

    add_menu_directory_item(
        string_load(30287) + " (" + show_x_filtered_items + ")",
        "plugin://plugin.video.nextembycon/?mode=WIDGET_CONTENT&type=recent_tvshows",
    )
    add_menu_directory_item(
        string_load(30263) + " (" + show_x_filtered_items + ")",
        "plugin://plugin.video.nextembycon/?mode=WIDGET_CONTENT&type=recent_episodes",
    )
    add_menu_directory_item(
        string_load(30264) + " (" + show_x_filtered_items + ")",
        "plugin://plugin.video.nextembycon/?mode=WIDGET_CONTENT&type=inprogress_episodes",
    )
    add_menu_directory_item(
        string_load(30265) + " (" + show_x_filtered_items + ")",
        "plugin://plugin.video.nextembycon/?mode=WIDGET_CONTENT&type=nextup_episodes",
    )
...
def show_search() -> None:
    add_menu_directory_item(
        string_load(30231),
        "plugin://plugin.video.nextembycon/?mode=NEW_SEARCH&item_type=Movie",
    )
    add_menu_directory_item(
        string_load(30229),
        "plugin://plugin.video.nextembycon/?mode=NEW_SEARCH&item_type=Series",
    )
    add_menu_directory_item(
        string_load(30235),
        "plugin://plugin.video.nextembycon/?mode=NEW_SEARCH&item_type=Episode",
    )
    add_menu_directory_item(
        string_load(30337),
        "plugin://plugin.video.nextembycon/?mode=NEW_SEARCH&item_type=Audio",
    )
    add_menu_directory_item(
        string_load(30338),
        "plugin://plugin.video.nextembycon/?mode=NEW_SEARCH&item_type=MusicAlbum",
    )
    add_menu_directory_item(
        string_load(30339),
        "plugin://plugin.video.nextembycon/?mode=NEW_SEARCH&item_type=Person",
    )
...
def set_library_window_values(force: bool = False) -> None:
    ...
    for item in result:
        ...
        if collection_type in ["movies", "boxsets", "music", "tvshows"]:
            ...
            log.debug(
                "set_library_window_values: plugin.video.nextembycon-{0}={1}",
                prop_name,
                name,
            )
