...
class DownloadUtils:
    ...
    def get_user_id(self) -> str:
        ...
        if userid and user_image:
            log.debug("Next EmbyCon DownloadUtils -> Returning saved UserID: {0}", userid)
            return userid
    ...
    def authenticate(self) -> str:
        ...
        if token is not None and token != "":
            log.debug(
                "Next EmbyCon DownloadUtils -> Returning saved AccessToken: {0}",
                "<redacted>",
            )
            return token
    ...
    def get_auth_header(self, authenticate: bool = True) -> dict[str, str]:
        ...
        log.debug("Next EmbyCon Authentication Header: {0}", redact_sensitive_data(headers))
        return headers
