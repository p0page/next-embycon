...
ADDON_ID = "plugin.video.nextembycon"
CLONED_SKIN_ID = "skin.estuary_nextembycon"


def check_skin_installed() -> None:
    params = {"addonid": CLONED_SKIN_ID, "properties": ["version", "enabled"]}
    result = JsonRpc("Addons.GetAddonDetails").execute(params)
    log.debug("Next EmbyCon Skin Details: {0}", result)
...
def clone_default_skin() -> None:
    ...
    if current_skin == CLONED_SKIN_ID:
        return

    message = []
    message.append("To switch to the new Next EmbyCon skin, go to:")
    message.append("Settings -> Interface -> Skin -> Choose Skin")
    message.append("and select 'Estuary Next EmbyCon' from the list.")
    xbmcgui.Dialog().ok("Next EmbyCon Skin Cloner", "\n".join(message))

    xbmc.executebuiltin("Dialog.Close(all,true)")
    xbmc.executebuiltin("ActivateWindow(interfacesettings)")

    # switch to the new skin
    # response = xbmcgui.Dialog().yesno("Next EmbyCon Skin Cloner",
    #                                  "Do you want to switch to the new cloned skin?")
    # if not response:
    #    return

    # log.debug("SkinCloner : Current Skin : " + get_value("lookandfeel.skin"))
    # set_result = set_value("lookandfeel.skin", CLONED_SKIN_ID)
...
def clone_skin() -> bool:
    ...
    embycon_path = os.path.join(kodi_home_path, "addons", ADDON_ID)
    ...
    pdialog.create("Next EmbyCon Skin Cloner", "")
    ...
    kodi_skin_destination = os.path.join(kodi_home_path, "addons", CLONED_SKIN_ID)
    ...
    addon_xml_data = addon_xml_data.replace(
        'id="skin.estuary"', 'id="{0}"'.format(CLONED_SKIN_ID)
    )
    addon_xml_data = addon_xml_data.replace('name="Estuary"', 'name="Estuary Next EmbyCon"')
    ...
    # enable the new skin
    params = {"addonid": CLONED_SKIN_ID, "enabled": True}
